...
void ConsistentSMMSourceOperator::Mult(const mfem::Vector &psi, mfem::Vector &source) const 
{
	...
	ProjectClosuresToFaces(psi_view, beta, tensor); 
...
void ConsistentSMMSourceOperator::ProjectClosuresToFaces(ConstTransportVectorView psi, 
	mfem::ParGridFunction &b, mfem::ParGridFunction &t) const 
{
	auto &mesh = *fes.GetParMesh(); 
	const auto dim = mesh.Dimension(); 
	const auto &beta_fes = *b.FESpace(); 
	const auto &tensor_fes = *t.FESpace(); 
	...
	for (auto e=0; e<fes.GetNE(); e++) {
		...
		for (auto f=0; f<element_to_face->RowSize(e); f++) {
			...
			for (auto n=0; n<ir.GetNPoints(); n++) {
				...
				b(beta_dof[local_face_id*tr_el.GetDof() + n]) = val; 
				for (auto d=0; d<dim; d++) {
					t(tensor_dof[local_face_id*tr_el.GetDof()*dim + n + d*tr_el.GetDof()]) 
